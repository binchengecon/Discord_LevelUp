...
class selfbot:
    def __init__(self, token):
        ...
        self.__class__.reactions = {":smiley:": "%F0%9F%98%83", ":grinning:": "%F0%9F%98%80",
                                    ":sob:": "%F0%9F%98%AD", ":laughing:": "%F0%9F%98%86"}
        self.token = token
        self.header = {"authorization": self.token}
        self.reactions = {":smiley:": "%F0%9F%98%83", ":grinning:": "%F0%9F%98%80",
                          ":sob:": "%F0%9F%98%AD", ":laughing:": "%F0%9F%98%86"}
    ...
    class get_user:
        ...
        def send_message(self, message):
            ...
            r = requests.post(
                f'https://discord.com/api/v9/users/@me/channels', json=data, headers=selfbot.header)
            print(r.status_code)
    ...
    class get_guild:
        ...
        def get_all_channels(self):
            r = requests.get(
                f'https://discordapp.com/api/v9/guilds/{self.guild_id}/channels', headers=selfbot.header)
        ...
        def delete_all_channels(self):
            response = requests.get(
                f"https://discord.com/api/guilds/{self.guild_id}/channels", headers=selfbot.header)
            data = response.json()

            for channel in data:
                r = requests.delete(
                    f"https://discord.com/api/v9/channels/{channel['id']}", headers=selfbot.header)

                if r.status_code == 200:
                    print(
                        f'200 SUCCESS in deleting channel: {channel["name"]}')
                else:
                    print(
                        f'{r.status_code} ERROR in deleting channel: {channel["name"]}')
